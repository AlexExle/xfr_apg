...
class XenForo_ControllerPublic_Member extends XenForo_ControllerPublic_Abstract
{
	public function actionIndex()
	{
		...
		$groupId = $this->_input->filterSingle('group', XenForo_Input::INT);

		if ($groupId > 0)
		{
			$users = $userModel->getUsers(array('user_group_id' => $groupId), array(
		...
		foreach ($users AS &$user){
			$awardedModel->prepareCachedData($user['xf_bdmedal_awarded_cached']);
		}
