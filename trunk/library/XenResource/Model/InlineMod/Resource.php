...
class XenResource_Model_InlineMod_Resource extends XenForo_Model
{
	...
	public function deleteResources(array $resourceIds, array $options = array(), &$errorKey = '', array $viewingUser = null)
	{
		...
		foreach ($resources AS $resource)
		{
			...
			if ($resource['resource_state'] == 'visible' && $options['authorAlert'])
			{
				$this->_getResourceModel()->sendModeratorActionAlert(
					'delete', $resource, $options['authorAlertReason']
				);
			}
		}
	...
	public function reassignResources(array $resourceIds, array $options = array(), &$errorKey = '', array $viewingUser = null)
	{
		...
		foreach ($resources AS $resource)
		{
			...
			)
			{
				...
				{
					$this->_getResourceModel()->sendModeratorActionAlert(
						'reassign_from', $resource, $options['alertReason'],
						array('to' => $options['username']), $oldUserId
					);
				}
				if ($newUserId)
				{
					$this->_getResourceModel()->sendModeratorActionAlert(
						'reassign_to', $resource, $options['alertReason'], array(), $newUserId
					);
				}
			}
		}
