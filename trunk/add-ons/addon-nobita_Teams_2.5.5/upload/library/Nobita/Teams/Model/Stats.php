...
class Nobita_Teams_Model_Stats extends Nobita_Teams_Model_Abstract
{
    ...
    {
        return array(
            'Nobita_Teams_GroupStatsHandler_Thread',
            'Nobita_Teams_GroupStatsHandler_Post',
            'Nobita_Teams_GroupStatsHandler_Member',
            'Nobita_Teams_GroupStatsHandler_Event',

            'Nobita_Teams_GroupStatsHandler_WallPost',
            'Nobita_Teams_GroupStatsHandler_Comment',
        );
    }
    ...
    /**
     * Deletes ALL data from the xf_team_stats_daily table. Use with care!
     */
    public function deleteStats()
    {
        $this->_getDb()->delete('xf_team_stats_daily');
    }
...
+			list($endYear, $endWeek) = explode('/', gmdate("o/W", $maxDate));
    ...
    public function getStatsData($teamId, $start, $end, array $statsTypes, $grouping = 'daily')
    {
        $db = $this->_getDb();
    ...
    public function buildStatsData($teamId, $start, $end)
    {
        $db = $this->_getDb();
        ...
        foreach ($this->_getStatsContentTypeHandlerNames() AS $contentType => $handlerClassName)
        {
            ...
            $data = $handlerClass->getData($teamId, $start, $end);
            ...
            foreach ($data AS $statsType => $records)
            {
                ...
                foreach ($records AS $date => $counter)
                {
                    ...
                    $db->query("
                        INSERT INTO xf_team_stats_daily
                            (team_id, stats_date, stats_type, counter)
                        VALUES
                            ($teamId, $date, $statsType, $counter)
