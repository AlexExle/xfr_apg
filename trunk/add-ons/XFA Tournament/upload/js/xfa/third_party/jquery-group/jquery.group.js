...
!function($, window, document, _undefined)
{
  ...
  standingsEditTemplate = standingsViewTemplate;//Handlebars.compile('<div class="standings"> <table> <colgroup> <col style="width: 40%"> <col span="5" style="width: 10%"> </colgroup> <tr><th></th><th>' + XenForo.phrases.xfa_tourn_points_abbrev + '</th><th>' + XenForo.phrases.xfa_tourn_wins_abbrev + '</th><th>' + XenForo.phrases.xfa_tourn_losses_abbrev + '</th><th>' + XenForo.phrases.xfa_tourn_ties_abbrev + '</th><th>&plusmn;</th></tr> {{#each this}} <tr><td><input class="name" type="text" data-prev="{{team.name}}" data-teamid="{{team.id}}" value="{{team.name}}" /></td>' + standingsScoreColumnMarkup + '</tr> {{/each}} </table> </div>');
  roundTemplate = Handlebars.compile('<div data-roundid="{{this}}" class="round"> {{#if this}} <header>' + XenForo.phrases.xfa_tourn_round + ' {{this}}</header> {{else}} <header>Unassigned</header> {{/if}} </div>');
